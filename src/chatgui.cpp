    ...
    : wxPanel(parent, -1, wxPoint(-1, -1), wxSize(-1, -1), wxBORDER_NONE)
{
    ...
    // create image and text; fall back to an empty bitmap if the chatbot has no image set
    wxBitmap displayImage = isFromUser ? wxBitmap(imgBasePath + "user.png", wxBITMAP_TYPE_PNG) : (bitmap ? *bitmap : wxBitmap());
    _chatBotImg = new wxStaticBitmap(this, wxID_ANY, displayImage, wxPoint(-1, -1), wxSize(-1, -1));
